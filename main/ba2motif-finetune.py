...
from dataset.ba2motif import BA2Motif
import model
...
parser.add_argument('--lr', default=8e-4, 
...
# In[Dataset]
train_dataset = BA2Motif(osp.join(args.datadir, f'BA-2Motifs/'), mode='train')
val_dataset = BA2Motif(osp.join(args.datadir, f'BA-2Motifs/'), mode='valid')
test_dataset = BA2Motif(osp.join(args.datadir, f'BA-2Motifs/'), mode='test')
...
# -2
# print('predictor.Encoder.node_dim:',predictor.Encoder.node_dim)
...
for epoch in range(1, epochs + 1):
    ...
    for graph_batch in train_loader:
        ...
        graph_batch.edge_index, graph_batch.exp_gt = remove_self_loops(graph_batch.edge_index,
                                                                       graph_batch.exp_gt)
        ...
        exp_pr, exp_re, exp_acc, dir_pr, roc_auc = exp_metric(explanation, graph_batch.exp_gt)
        ...
        exp_loss = Exp_loss(explanation, graph_batch.exp_gt.float())
    ...
    if epoch % 2 == 0:
        ...
        with torch.no_grad():
            for graph_batch in val_loader:
                ...
                graph_batch.edge_index, graph_batch.exp_gt = remove_self_loops(graph_batch.edge_index,
                                                                       graph_batch.exp_gt)
                explanation, edge_pool = explainer(graph_batch, 0)
                prediction = predictor(graph_batch, explanation, edge_pool)
            ...
            if max_val_loss > np.mean(val_total_loss):
                ...
                torch.save(explainer.cpu(), osp.join('finetuned/BA2Motifs/Explainer.pt'))
                torch.save(projector.cpu(), osp.join('finetuned/BA2Motifs/Projector.pt'))
                torch.save(predictor.cpu(), osp.join('finetuned/BA2Motifs/Predictor.pt'))
                explainer.to(device)
                projector.to(device)
                predictor.to(device)
...
# In[Test]
best_explainer = torch.load('finetuned/BA2Motifs/Explainer.pt').to(device)
best_projector = torch.load('finetuned/BA2Motifs/Projector.pt').to(device)
best_predictor = torch.load('finetuned/BA2Motifs/Predictor.pt').to(device)
test_right = 0
test_exp_pr = []
test_exp_re = []
test_exp_acc = []
...
with torch.no_grad():
    for graph_batch in test_loader:
        ...
        graph_batch.edge_index, graph_batch.exp_gt = remove_self_loops(graph_batch.edge_index,
                                                                    graph_batch.exp_gt)
        explanation, edge_pool = best_explainer(graph_batch, 0)
