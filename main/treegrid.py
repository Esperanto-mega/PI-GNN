...
# In[Save path]
save_path = '/home/data/yj/Bnova/trained/TreeGrid'
...
for epoch in range(1, epochs + 1):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(predictor.cpu(),osp.join('trained/TreeGrid/Predictor.pt'))
        predictor.to(device)
    if val_auc > best_val_auc:
        print('Validate explanation improved.')
        best_val_auc = val_auc
        torch.save(e2rfunction.cpu(), osp.join('trained/TreeGrid/E2RFunction.pt'))
        torch.save(hyperweight.cpu(), osp.join('trained/TreeGrid/Weightor.pt'))
...
# In[Test]
best_e2rfunction = torch.load('trained/TreeGrid/E2RFunction.pt').to(device)
best_weightor = torch.load('trained/TreeGrid/Weightor.pt').to(device)
best_predictor = torch.load('trained/TreeGrid/Predictor.pt').to(device)
