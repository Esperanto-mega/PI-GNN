...
for epoch in range(1, epochs + 1):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(predictor.cpu(),osp.join('trained/TreeCycles/Predictor.pt'))
        predictor.to(device)
    if val_auc > best_val_auc:
        print('Validate explanation improved.')
        best_val_auc = val_auc
        torch.save(e2rfunction.cpu(), osp.join('trained/TreeCycles/E2RFunction.pt'))
        torch.save(hyperweight.cpu(), osp.join('trained/TreeCycles/Weightor.pt'))
...
# In[Test]
best_e2rfunction = torch.load('trained/TreeCycles/E2RFunction.pt').to(device)
best_weightor = torch.load('trained/TreeCycles/Weightor.pt').to(device)
best_predictor = torch.load('trained/TreeCycles/Predictor.pt').to(device)
