...
class Synthetic(InMemoryDataset):
    ...
    def __init__(self, root, mode='train', transform=None, pre_transform=None, pre_filter=None):
        ...
        self.mode = mode
        super(Synthetic, self).__init__(root, transform, pre_transform, pre_filter)

        idx = self.processed_file_names.index('synthetic_{}.pt'.format(mode))
    ...
    def download(self):
        if not osp.exists(osp.join(self.raw_dir, 'raw', 'synthetic_train.npy')):
            print("raw data of `Synthetic` doesn't exist, please redownload from our github.")
    ...
    def process(self):
        ...
        for idx, (edge_index, y, ground_truth, z, p) in enumerate(zip(edge_index_list, label_list, ground_truth_list, role_id_list, pos)):
            ...
            data = Data(x=x, y=y, z=z,
                        ...
                        name=f'Synthetic_-{self.mode}-{idx}', idx=idx)
        ...
        idx = self.processed_file_names.index('synthetic_{}.pt'.format(self.mode))
